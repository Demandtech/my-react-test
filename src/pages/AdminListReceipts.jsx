...
const AdminListReceipts = () => {
	...
	// Context
	const { dispatch, isLoading } = React.useContext(AuthContext);
	const {
		...
		<div className="overflow-x-auto  rounded bg-white p-5 shadow">
			...
			<ActionConfirmationModal
				...
				onSuccess={(data) => {
					...
					setLocalReceiptData((prevData) => {
						const updatedData = prevData.map((item) =>
							item.id === data.id ? { ...item, receipt_status: 1 } : item
						);
			...
			<ActionConfirmationModal
				...
				onSuccess={(data) => {
					setLocalReceiptData((prevData) => {
						const updatedData = prevData.map((item) =>
							item.id === data.id ? { ...item, receipt_status: 0 } : item
						);
